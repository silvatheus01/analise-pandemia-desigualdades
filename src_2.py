...
##############
# bd.download(
#     savepath='br_inep_censo_escolar__escola.csv',
#     # limit=1,
#     # dataset_id='br_inep_censo_escolar',
#     # table_id='escola',
#     query = 'SELECT * FROM basedosdados.br_inep_censo_escolar.escola LIMIT 100',
#     billing_project_id=PROJECT_ID,
# )
#for ano in range(2018,2024):
df = bd.read_sql(
    query=f'SELECT * FROM `basedosdados.br_inep_censo_escolar.escola` LIMIT 100',
    billing_project_id=PROJECT_ID
)

df.to_excel(f'br_inep_censo_escolar__escola.xlsx', index=False)

# df.to_csv('br_inep_censo_escolar.csv')
